    ...
    async def approve(self, button: nextcord.ui.button, interaction: Interaction):
        ...
        if isthere:
            ...
            if interaction.user.nick is not None:
                await user.send(f"Approved! Your Proof Of Work was accepted by {interaction.user.nick}.")
            else:
                await user.send(f"Approved! Your Proof Of Work was accepted by {interaction.user.name}.")
            await interaction.response.send_message(f"{interaction.user.mention} approved {user.mention} PoW.")
...
class Modal(nextcord.ui.Modal):
    ...
    async def callback(self, interaction: Interaction):
        ...
        success = database.add_message(interaction, message.id)
        if success == 0:
            await interaction.response.send_message(embed=SENT_FOR_VAILDATION_EMBED, ephemeral=EPHEMERAL)
        else:
            await interaction.response.send_message(embed=EXCESS_VALIDATION_EMBED, ephemeral=EPHEMERAL)
...
async def submit_flag(interaction: Interaction, flag:str):
    ...
    if flag != ans_flag:
        ...
        await interaction.response.send_message(embed=INCORRECT_FLAG_EMBED)
        ...
        if _submit == 1:              
            await interaction.response.send_message(embed=REPEATED_SUBMIT_EMBED)
        else:
            modal = Modal()
